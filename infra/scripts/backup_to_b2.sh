...
cleanup_pgpass() {
  b2_backup_run_database_client \
    env -u PGPASSWORD -u POSTGRES_PASSWORD \
    docker exec "${DB_CONTAINER}" rm -f "${pgpass_path}" 2>/dev/null || true
}
...
b2_backup_run_database_client docker \
    exec -e PGPASSFILE="${pgpass_path}" "${DB_CONTAINER}" pg_dump \
    -U "${PGUSER}" \
    -d "${PGDATABASE}" \
    --format=custom \
    --compress=6 \
