...
def test_build_argument_parser_requires_year() -> None:
    with pytest.raises(SystemExit, match="2"):
        cli._build_argument_parser().parse_args(["--path", "/tmp/sample.csv", "--data-type", "contributions"])


def test_build_argument_parser_accepts_all_pa_data_types() -> None:
    for data_type in ("contributions", "expenditures", "debts", "receipts", "filings"):
        args = cli._build_argument_parser().parse_args(
            ["--year", "2025", "--path", "/tmp/sample.csv", "--data-type", data_type]
        )
...
def test_build_argument_parser_rejects_path_and_download_together() -> None:
    with pytest.raises(SystemExit, match="2"):
        cli._build_argument_parser().parse_args(
            ["--year", "2025", "--path", "/tmp/sample.csv", "--download", "--data-type", "contributions"]
        )


def test_build_argument_parser_parses_limit_and_dry_run() -> None:
    args = cli._build_argument_parser().parse_args(
        ["--year", "2025", "--path", "/tmp/sample.csv", "--data-type", "contributions", "--limit", "5", "--dry-run"]
    )
...
def test_main_download_mode_uses_downloaded_archive_path(
    ...
    downloaded_archive = tmp_path / "PA_2025.zip"

    monkeypatch.setattr(cli, "get_connection", MagicMock(return_value=connection))
    download_pa_archive = MagicMock(return_value=downloaded_archive)
    monkeypatch.setattr(cli, "download_pa_archive", download_pa_archive)
    load_contributions = MagicMock(return_value=load_result)
    monkeypatch.setattr(cli, "load_pa_contributions_with_filings", load_contributions)

    exit_code = cli.main(["--year", "2025", "--download", "--data-type", "contributions"])
    captured = capsys.readouterr()

    assert exit_code == 0
    download_pa_archive.assert_called_once_with("contributions", 2025, ANY)
    load_contributions.assert_called_once_with(connection, downloaded_archive, year=2025, limit=None)
    assert "PA contributions load complete" in captured.out
...
def test_main_dry_run_reports_row_count_without_db(
    ...
    exit_code = cli.main(
        ["--year", "2025", "--path", str(_SAMPLE_CONTRIBUTIONS_PATH), "--data-type", "contributions", "--dry-run"]
    )
    captured = capsys.readouterr()

    assert exit_code == 0
    cli.get_connection.assert_not_called()
    assert "PA contributions dry-run: parsed 3 rows" in captured.out
...
def test_main_returns_non_zero_for_load_failure(
    ...
    exit_code = cli.main(["--year", "2025", "--path", str(_SAMPLE_CONTRIBUTIONS_PATH), "--data-type", "contributions"])
    captured = capsys.readouterr()

    assert exit_code == 1
    assert "PA ingest failed: load failed" in captured.err


def test_run_pa_refresh_executes_typed_path_mode(monkeypatch: pytest.MonkeyPatch) -> None:
    connection = MagicMock()
    load_result = _build_load_result()
    ...
    assert resolved_args.year == 2025
    ...
    load_path.assert_called_once_with(
        connection, _SAMPLE_CONTRIBUTIONS_PATH, data_type="contributions", year=2025, limit=6
    )
...
def test_run_pa_refresh_rejects_filings_data_type(monkeypatch: pytest.MonkeyPatch) -> None:
