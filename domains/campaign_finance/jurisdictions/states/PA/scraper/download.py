"""
Stub summary for /Users/stuart/parallel_development/civibus_dev/mar21_02_tx_pa_state_pipelines/civibus_dev/domains/campaign_finance/jurisdictions/states/PA/scraper/download.py.
"""
...
_PA_ARCHIVE_FILENAME_TEMPLATE = "PA_{year}.zip"
                    ...
                    for chunk in response.iter_bytes():
                        if chunk:
                            ...
                            if downloaded_bytes > MAX_DOWNLOAD_BYTES:
                                raise ValueError(
                                    f"PA download exceeds the allowed size limit of {MAX_DOWNLOAD_BYTES} bytes"
                                )
...
def download_pa_archive(data_type: str, year: int, dest_dir: Path) -> Path:
    download_url = _load_bulk_download_url_for_data_type(data_type, year)
    destination_path = dest_dir / _PA_ARCHIVE_FILENAME_TEMPLATE.format(year=year)
