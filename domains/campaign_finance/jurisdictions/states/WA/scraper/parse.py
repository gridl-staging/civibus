"""
Stub summary for MAR18_state_expansion_batch_2/civibus_dev/domains/campaign_finance/jurisdictions/states/WA/scraper/parse.py.
"""
...
INDEPENDENT_EXPENDITURE_COLUMNS = _load_columns_for_data_type("independent_expenditures")
...
def parse_contributions(path: Path) -> WACsvParser:
    return WACsvParser(path=path, columns=CONTRIBUTION_COLUMNS, row_label="contribution")


def parse_expenditures(path: Path) -> WACsvParser:
    return WACsvParser(path=path, columns=EXPENDITURE_COLUMNS, row_label="expenditure")


def parse_loans(path: Path) -> WACsvParser:
    return WACsvParser(path=path, columns=LOAN_COLUMNS, row_label="loan")


def parse_independent_expenditures(path: Path) -> WACsvParser:
    return WACsvParser(
        path=path,
        columns=INDEPENDENT_EXPENDITURE_COLUMNS,
        row_label="independent_expenditure",
    )
