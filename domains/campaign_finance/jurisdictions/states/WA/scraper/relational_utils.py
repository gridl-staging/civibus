...
class WASourceRecordKeyLedger:
    ...
    def blocks_relational_link(self, row_key: str) -> bool:
        """Return whether every attempt for this key failed to persist provenance."""
        return row_key in self.rejected_attempts and row_key not in self.persisted

    def rejected_attempts_for_persisted_keys(self) -> int:
        """Count failed attempts whose content another row successfully persisted."""
        return sum(self.rejected_attempts[row_key] for row_key in self.persisted)
...
def _link_wa_relational_row(
    ...
    *,
    ...
    if settings.key_ledger is not None and settings.key_ledger.blocks_relational_link(row_key):
        return
