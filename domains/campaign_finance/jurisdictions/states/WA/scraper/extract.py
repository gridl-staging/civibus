...
class WAContributionExtraction(TypedDict):
...
class WAExpenditureExtraction(TypedDict):
...
class WALoanExtraction(TypedDict):
...
def _contribution_fields() -> dict[str, str]:
    return {
        ...
        "donor_address": _load_column_for_semantic_path("contributions", "donor.address.street1"),
...
def _loan_fields() -> dict[str, str]:
    ...
    return {
        ...
        for field_name, semantic_path in {
            ...
            "lender_employer": "lender.employer",
            "lender_occupation": "lender.occupation",
            "address1": "lender.address.street1",
...
def extract_wa_contribution(row: dict[str, str | None]) -> WAContributionExtraction:
    ...
    donor_person = None
    donor_org = None
    if _is_individual_party(donor_type, donor_name):
        donor_person = _person_from_full_name(
            donor_name,
            identifiers={
                "employer": _normalized_text(row.get(fields["donor_employer"])),
                "occupation": _normalized_text(row.get(fields["donor_occupation"])),
            },
        )
    elif donor_name is not None:
        donor_org = Organization(canonical_name=donor_name)
    ...
    return {
        ...
        "address": _extract_street_address(
            address1=row.get(fields["donor_address"]),
            city=row.get(fields["donor_city"]),
            state=row.get(fields["donor_state"]),
            raw_zip=row.get(fields["donor_zip"]),
        ),
    }


def extract_wa_expenditure(row: dict[str, str | None]) -> WAExpenditureExtraction:
    return _extract_wa_payee_transaction(row, fields=_expenditure_fields())


def extract_wa_independent_expenditure(
    row: dict[str, str | None],
) -> WAExpenditureExtraction:
    return _extract_wa_payee_transaction(row, fields=_independent_expenditure_fields())


def _extract_wa_payee_transaction(
    row: dict[str, str | None],
    *,
    fields: dict[str, str],
) -> WAExpenditureExtraction:
...
def extract_wa_loan(row: dict[str, str | None]) -> WALoanExtraction:
    fields = _loan_fields()
    lender_name = _normalized_text(row.get(fields["lender_name"]))
    lender_type = _normalized_text(row.get(fields["lender_type"]))

    lender_person = None
    lender_org = None
    if _is_individual_party(lender_type, lender_name):
        lender_person = _person_from_full_name(
            lender_name,
            identifiers={
                "employer": _normalized_text(row.get(fields["lender_employer"])),
                "occupation": _normalized_text(row.get(fields["lender_occupation"])),
            },
        )
    elif lender_name is not None:
        lender_org = Organization(canonical_name=lender_name)
...
def _extract_committee(row: dict[str, str | None], *, fields: dict[str, str]) -> Organization:
    ...
    identifiers = {"wa_committee_id": normalized_committee_id} if normalized_committee_id is not None else {}
...
def _extract_street_address(
    *,
    ...
    if not any((normalized_address1, normalized_city, normalized_state, normalized_zip)):
        return None

    zip5, zip4 = _split_zip(normalized_zip)
    raw_address = ", ".join(
        part for part in (normalized_address1, normalized_city, normalized_state, normalized_zip) if part
    )
    return Address(
...
def _split_person_name(full_name: str) -> tuple[str | None, str | None]:
    if "," in full_name:
        ...
        first_name = _title_case_name(first_name_part.split(" ")[0])
...
def _is_individual_party(category: str | None, name: str | None) -> bool:
    normalized_category = _normalized_text(category)
    if normalized_category is not None and normalized_category.lower().startswith("individual"):
        return True

    normalized_name = _normalized_text(name)
    return normalized_name is not None and "," in normalized_name
...
def _split_zip(raw_zip: str | None) -> tuple[str | None, str | None]:
    ...
    if "-" in normalized_zip:
        ...
        zip5 = _normalized_digits(zip5)
        return zip5 if is_valid_zip5(zip5) else None, _normalized_digits(zip4)

    # 9-digit all-numeric: treat as concatenated zip5+zip4
    digits = _normalized_digits(normalized_zip)
    if digits is not None and len(digits) == 9:
        return digits[:5], digits[5:]

    # Only valid as zip5 if exactly 5 digits
    return digits if is_valid_zip5(digits) else None, None
