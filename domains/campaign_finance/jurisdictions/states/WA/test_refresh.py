...
from domains.campaign_finance.jurisdictions.states.WA import refresh as wa_refresh
from domains.campaign_finance.jurisdictions.states.WA.scraper.cli import run_wa_refresh

NOW = datetime(2026, 8, 25, 12, 0, tzinfo=timezone.utc)
CONFIG_PATH = Path(__file__).with_name("config.yaml")
EXPECTED_JOBS = (
    ("state-wa-contributions", "contributions"),
    ("state-wa-expenditures", "expenditures"),
    ("state-wa-independent_expenditures", "independent_expenditures"),
    ("state-wa-loans", "loans"),
)
...
def test_build_refresh_jobs_matches_wa_package_api() -> None:
    config = _load_config()
    jobs = wa_refresh.build_refresh_jobs(config, RunnerParameters(), NOW)
    ...
    for job, (_key, data_type) in zip(jobs, EXPECTED_JOBS, strict=True):
        ...
        assert job.jurisdiction == "state/WA"
        assert job.cadence == source.update_frequency
        assert job.data_source_names == (source.name,)
        assert job.run_callable.func is run_wa_refresh
        assert job.run_callable.keywords == {"data_type": data_type, "download": True}

    alternate_jobs = wa_refresh.build_refresh_jobs(_load_config(jurisdiction_code="ZZ"), RunnerParameters(), NOW)
...
def test_build_refresh_jobs_omits_only_missing_source() -> None:
    config = _load_config()
    jobs = wa_refresh.build_refresh_jobs(
        _without_data_type(config, "independent_expenditures"), RunnerParameters(), NOW
    )

    assert [job.key for job in jobs] == ["state-wa-contributions", "state-wa-expenditures", "state-wa-loans"]
