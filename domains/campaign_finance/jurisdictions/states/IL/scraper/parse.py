...
class ILDelimitedParser:
    """Iterate IL tab-delimited rows as normalized dictionaries."""
    ...
    def __iter__(self) -> Iterator[dict[str, str | None]]:
        ...
        with self.path.open("r", encoding="utf-8", errors="replace", newline="") as delimited_file:
...
def _validate_header(fieldnames: list[str] | None, expected_columns: tuple[str, ...], row_label: str) -> None:
    if tuple(fieldnames or ()) != expected_columns:
        raise ValueError(f"Unexpected {row_label} header")
...
def _normalize_row(raw_row: dict[object, object]) -> dict[str, str | None]:
    ...
    for key, value in raw_row.items():
        ...
        if not isinstance(value, str):
            raise ValueError(f"Unexpected non-string IL value for {key!r}: {type(value)!r}")
        normalized_row[str(key)] = value

    return normalized_row


def parse_contributions(path: Path) -> ILDelimitedParser:
    return ILDelimitedParser(path=path, columns=CONTRIBUTION_COLUMNS, row_label="contribution")


def parse_expenditures(path: Path) -> ILDelimitedParser:
    return ILDelimitedParser(path=path, columns=EXPENDITURE_COLUMNS, row_label="expenditure")
