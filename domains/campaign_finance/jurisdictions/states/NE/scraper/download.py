                    ...
                    for chunk in response.iter_bytes():
                        if chunk:
                            ...
                            if downloaded_bytes > MAX_DOWNLOAD_BYTES:
                                raise ValueError(
                                    f"NE download exceeds the allowed size limit of {MAX_DOWNLOAD_BYTES} bytes"
                                )
...
def download_ne_archive(data_type: str, year: int, dest_dir: Path) -> Path:
    normalized_data_type = data_type.strip().lower()
    download_url = _load_bulk_download_url_for_data_type(normalized_data_type, year)
    destination_path = dest_dir / f"NE_{normalized_data_type}_{year}.zip"
