...
@lru_cache(maxsize=1)
def _load_ne_config() -> JurisdictionConfig:
    return load_state_config(_CONFIG_PATH, state_code="NE")


@lru_cache(maxsize=1)
def _load_ne_data_source_blocks() -> tuple[DataSourceBlock, ...]:
    return build_data_source_blocks(_load_ne_config())


def _load_data_source_for_data_type(data_type: str) -> DataSourceBlock:
    return load_data_source_for_data_type(
        _load_ne_data_source_blocks(),
        data_type=data_type,
        state_code="NE",
    )
...
def load_supported_data_types() -> tuple[str, ...]:
    return load_block_supported_data_types(_load_ne_data_source_blocks())
...
def _load_bulk_download_url_for_data_type(data_type: str, year: int) -> str:
    ...
    if template is None:
        raise RuntimeError(f"NE config missing bulk_download_url for data type {data_type!r}")

    if "{year}" not in template:
        raise RuntimeError(
            f"NE bulk_download_url for data type {data_type!r} must include '{{year}}' placeholder: {template!r}"
        )
    ...
    try:
        ...
        raise RuntimeError(
            f"NE bulk_download_url for data type {data_type!r} is not a valid year template: {template!r}"
        ) from error
