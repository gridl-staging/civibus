...
from domains.campaign_finance.jurisdictions.states.NE import refresh as ne_refresh
from domains.campaign_finance.jurisdictions.states.NE.scraper.cli import run_ne_refresh

NOW = datetime(2026, 8, 25, 12, 0, tzinfo=timezone.utc)
CONFIG_PATH = Path(__file__).with_name("config.yaml")
EXPECTED_JOBS = (
    ("state-ne-contributions", "contributions"),
    ("state-ne-loans", "loans"),
    ("state-ne-expenditures", "expenditures"),
)
...
def test_build_refresh_jobs_matches_ne_package_api() -> None:
    config = _load_config()
    jobs = ne_refresh.build_refresh_jobs(config, RunnerParameters(), NOW)
    ...
    for job, (_key, data_type) in zip(jobs, EXPECTED_JOBS, strict=True):
        ...
        assert job.jurisdiction == "state/NE"
        assert job.cadence == source.update_frequency
        assert job.data_source_names == (source.name,)
        assert job.run_callable.func is run_ne_refresh
        assert job.run_callable.keywords == {"data_type": data_type, "download": True, "year": 2026}

    alternate_jobs = ne_refresh.build_refresh_jobs(
    ...
    assert all(job.run_callable.keywords["year"] == 2031 for job in alternate_jobs)


def test_build_refresh_jobs_omits_only_missing_source() -> None:
    config = _load_config()
    jobs = ne_refresh.build_refresh_jobs(_without_data_type(config, "expenditures"), RunnerParameters(), NOW)

    assert [job.key for job in jobs] == ["state-ne-contributions", "state-ne-loans"]
