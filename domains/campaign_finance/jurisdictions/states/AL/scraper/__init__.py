...
def _load_al_config() -> JurisdictionConfig:
    return load_state_config(_CONFIG_PATH, state_code="AL")


@lru_cache(maxsize=1)
def _load_al_data_source_blocks() -> tuple[DataSourceBlock, ...]:
    return build_data_source_blocks(_load_al_config())


def _load_data_source_for_data_type(data_type: str) -> DataSourceBlock:
    return load_data_source_for_data_type(
        _load_al_data_source_blocks(),
        data_type=data_type,
        state_code="AL",
    )
...
def load_supported_data_types() -> tuple[str, ...]:
    return load_block_supported_data_types(_load_al_data_source_blocks())
...
def _load_api_base_url_for_data_type(data_type: str) -> str:
    ...
    if data_source.api_base_url is None:
        raise RuntimeError(f"AL config missing api_base_url for data type {data_type!r}")
