...
_AL_DOMAIN = "campaign_finance"
_AL_JURISDICTION = "state/AL"
_AL_SOURCE_FORMAT = "json"
...
class _ALFilingLookupEntry:
    filing_id: UUID
    committee_id: UUID
    source_record_id: UUID


_AL_ENTITY_KEYS: dict[str, tuple[str, str]] = {
    "contributions": ("donor_person", "donor_org"),
    "expenditures": ("payee_person", "payee_org"),
}

_AL_EXTRACT_FN = {
    "contributions": extract_al_contribution,
    "expenditures": extract_al_expenditure,
}
...
_AL_ENTITY_ROLES: dict[str, dict[str, str]] = {
    "contributions": {
        "person": "donor",
...
_AL_COUNTERPARTY_ROLES: dict[str, tuple[tuple[str, ...], tuple[str, ...]]] = {
    "contributions": (("donor",), ("donor_org",)),
    "expenditures": (("payee",), ("payee_org",)),
}

_AL_TRANSACTION_KIND: dict[str, str] = {
...
def ensure_al_data_source(conn: psycopg.Connection, data_type: str) -> UUID:
    """Ensure the AL data source row exists in the DB and return its UUID."""
    normalized = data_type.strip().lower()
    return ensure_data_source(
        conn,
        DataSource(
            domain=_AL_DOMAIN,
            jurisdiction=_AL_JURISDICTION,
            name=_load_data_source_name_for_data_type(normalized),
            source_url=_load_data_source_url_for_data_type(normalized),
            source_format=_AL_SOURCE_FORMAT,
        ),
    )


def _al_source_record_key(row: Mapping[str, str | None]) -> str:
    return compute_record_hash(dict(row))


def _build_al_source_record(
    data_source_id: UUID,
...
def _resolve_al_committee_id(conn: psycopg.Connection, committee_org: object) -> UUID:
    """Resolve committee to an organization UUID, using al_org_id identifier if available."""
    ...
    al_org_id = _normalize_optional_text(committee_org.identifiers.get("al_org_id"))
    if al_org_id is not None:
        existing = find_organization_by_identifier(conn, "al_org_id", al_org_id)
        if existing is not None:
            return existing
    return resolve_organization_by_canonical_name(conn, committee_org)


def _load_al_transaction_entities(
    ...
    *,
    ...
    """Insert entity_source and entity_address linkage for a single row's entities."""
    roles = _AL_ENTITY_ROLES[data_type]
    person_key, org_key = _AL_ENTITY_KEYS[data_type]
    ...
    committee_id = _resolve_al_committee_id(conn, committee)
    link_entity_source_and_optional_mailing_address(
        conn,
...
def _extract_and_load_al_row(
    ...
    *,
    ...
    extract_fn = _AL_EXTRACT_FN.get(data_type)
    if extract_fn is None:
        raise ValueError(f"Unsupported AL data type: {data_type}")

    source_record_id = try_insert_source_record(
        conn,
        _build_al_source_record(data_source_id, row, data_type=data_type),
    )
    if source_record_id is None:
        return False  # Duplicate row — already loaded.

    extracted = extract_fn(dict(row))
    _load_al_transaction_entities(
        conn,
        source_record_id=source_record_id,
        extracted=extracted,
        data_type=data_type,
...
def _try_load_row(
    ...
    *,
    ...
    """Attempt to load a single row with per-row savepoint protection."""
    try:
        if manages_outer_transaction:
            ensure_transaction_open(conn)
        with conn.transaction():
            return _extract_and_load_al_row(conn, row, data_source_id, data_type=data_type)
    except Exception:
        LOGGER.exception("Failed loading AL %s row", data_type.rstrip("s"))
        return None
...
def _load_al_rows(
    ...
    *,
    ...
    """Insert source records and entities for a stream of parsed rows."""
    started_at = time.monotonic()
    counts = _ALLoadCounts()
...
def _load_al_file(
    ...
    *,
    ...
    parser = _AL_PARSER_FN[data_type](Path(file_path), year_from=year_from)
    return _load_al_rows(
        conn,
        parser,
        data_source_id=data_source_id,
        data_type=data_type,
...
def _parse_optional_al_date(raw_value: str | None) -> date | None:
    """Parse an AL date string (MM/DD/YYYY) to a date object, or None."""
    ...
    for fmt in ("%m/%d/%Y", "%m/%d/%y", "%Y-%m-%dT%H:%M:%S"):
        try:
            ...
            continue
    raise ValueError(f"AL row has invalid date: {raw_value}")


def _required_al_text(value: str | None, field_name: str) -> str:
    normalized = _normalize_optional_text(value)
    if normalized is None:
        raise ValueError(f"AL row missing {field_name}")
    return normalized


def _parse_required_al_amount(raw_value: str | None, field_name: str) -> Decimal:
    """Parse a currency amount string to Decimal, stripping $ and commas."""
    normalized = _required_al_text(raw_value, field_name)
    try:
        return Decimal(normalized.replace(",", "").replace("$", ""))
    except InvalidOperation as exc:
        raise ValueError(f"AL row has invalid {field_name}: {raw_value}") from exc


def _amendment_indicator(raw_value: str | None) -> str:
    """Map AL 'amended' field to standard amendment indicator (N/A/T)."""
    normalized = _normalize_optional_text(raw_value)
    if normalized is None:
        return "N"
...
def _build_al_filing_fec_id(row: Mapping[str, str | None], data_type: str) -> str:
    ...
    return f"AL-{committee_id}-{filed_date}-{data_type}"


def _resolve_al_filing_committee_id(
    conn: psycopg.Connection,
    row: Mapping[str, str | None],
    data_type: str,
) -> UUID:
    """Resolve the committee for a filing row to a cf.committee UUID."""
    extracted = _AL_EXTRACT_FN[data_type](dict(row))
    org_id = _resolve_al_committee_id(conn, extracted["committee"])
    committee_id_col = _load_column_for_semantic_path(data_type, "committee.id")
    native_id = _normalize_optional_text(row.get(committee_id_col))
    if native_id is None:
        native_id = _build_al_filing_fec_id(row, data_type)
...
def _upsert_al_filing(
    conn: psycopg.Connection,
    row: Mapping[str, str | None],
    *,
    source_record_id: UUID,
    data_type: str,
    filing_lookup: dict[str, _ALFilingLookupEntry],
) -> _ALFilingLookupEntry:
    """Upsert a filing for the given row, caching in filing_lookup."""
    filing_fec_id = _build_al_filing_fec_id(row, data_type)
    existing = filing_lookup.get(filing_fec_id)

    if existing is None:
        committee_id = _resolve_al_filing_committee_id(conn, row, data_type)
        filing_src = source_record_id
    else:
        committee_id = existing.committee_id
        filing_src = existing.source_record_id

    filing = _build_al_filing(
        row,
        committee_id=committee_id,
...
def _select_al_source_record_id(
    conn: psycopg.Connection,
    *,
    data_source_id: UUID,
...
def _resolve_al_transaction_address_id(
    conn: psycopg.Connection,
    *,
    source_record_id: UUID,
    data_type: str,
) -> UUID | None:
    address_role = _AL_ENTITY_ROLES[data_type]["address"]
    with conn.cursor() as cursor:
        cursor.execute(
            """
            SELECT entity_id
...
def _upsert_al_transaction_with_filing(
    conn: psycopg.Connection,
    row: Mapping[str, str | None],
    *,
    filing_id: UUID,
    ...
    """Build and upsert the Transaction record for a single row."""
    person_roles, org_roles = _AL_COUNTERPARTY_ROLES[data_type]
    ...
    address_id = _resolve_al_transaction_address_id(
        conn,
        source_record_id=source_record_id,
        data_type=data_type,
    )
...
def _load_al_relational_transactions(
    conn: psycopg.Connection,
    rows: Iterable[Mapping[str, str | None]],
    *,
    data_source_id: UUID,
    ...
    """Second pass: create Filing and Transaction records for previously-inserted source records."""
    filing_lookup: dict[str, _ALFilingLookupEntry] = {}
    relational_errors = 0
    manages_outer = conn.info.transaction_status == psycopg.pq.TransactionStatus.IDLE
    ...
    for index, row in enumerate(rows, start=1):
        ...
        source_record_id = _select_al_source_record_id(
            conn,
            data_source_id=data_source_id,
            source_record_key=_al_source_record_key(row),
        )
        if source_record_id is None:
            continue
        ...
        try:
            if manages_outer:
                ensure_transaction_open(conn)
            with conn.transaction():
                filing_entry = _upsert_al_filing(
                    conn,
                    row,
                    source_record_id=source_record_id,
                    data_type=data_type,
                ...
                _upsert_al_transaction_with_filing(
                    conn,
                    row,
            ...
            relational_errors += 1
            LOGGER.exception("Failed linking AL %s row to filing", data_type.rstrip("s"))

    commit_managed_transaction(conn, manages_outer)
    return relational_errors


def _load_al_with_filings(
    conn: psycopg.Connection,
    fp: str | Path,
    *,
    data_type: str,
    ...
    load_result.errors += _load_al_relational_transactions(
        conn,
        _AL_PARSER_FN[data_type](Path(fp), year_from=year_from),
        data_source_id=data_source_id,
        data_type=data_type,
        limit=validated_row_limit,
    )
...
def load_al_contributions_with_filings(
    conn: psycopg.Connection,
    fp: str | Path,
    *,
    year_from: int | None = None,
    ...
    """Load AL contributions with filings and transactions."""
    return _load_al_with_filings(conn, fp, data_type="contributions", year_from=year_from, limit=limit)


def load_al_expenditures_with_filings(
    conn: psycopg.Connection,
    fp: str | Path,
    *,
    year_from: int | None = None,
    ...
    """Load AL expenditures with filings and transactions."""
    return _load_al_with_filings(conn, fp, data_type="expenditures", year_from=year_from, limit=limit)
