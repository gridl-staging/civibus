...
from domains.campaign_finance.jurisdictions.states.AL.scraper import cli
...
def test_build_argument_parser_accepts_all_al_data_types() -> None:
    parser = cli._build_argument_parser()
    for data_type in ("contributions", "expenditures"):
        args = parser.parse_args(["--path", "/tmp/sample.json", "--data-type", data_type])
...
def test_build_argument_parser_rejects_path_and_download_together() -> None:
    with pytest.raises(SystemExit, match="2"):
        cli._build_argument_parser().parse_args(
            ["--path", "/tmp/sample.json", "--download", "--data-type", "contributions"]
        )


def test_run_al_refresh_executes_path_mode(monkeypatch) -> None:
...
def test_run_al_refresh_requires_path_or_download() -> None:
    with pytest.raises(ValueError, match="requires either path or download mode"):
        cli.run_al_refresh(data_type="contributions")


def test_run_al_refresh_rejects_path_and_download_together() -> None:
    with pytest.raises(ValueError, match="accepts path or download mode, not both"):
        cli.run_al_refresh(data_type="contributions", path=Path("/tmp/x.json"), download=True)
...
def test_main_dry_run_reports_row_count_without_db(monkeypatch, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert "AL contributions dry-run: parsed 3 rows" in captured.out


def test_main_returns_1_on_error(monkeypatch, capsys: pytest.CaptureFixture[str]) -> None:
    monkeypatch.setattr(cli, "_resolve_input_path", MagicMock(side_effect=RuntimeError("test error")))

    exit_code = cli.main(["--path", str(_SAMPLE_CONTRIBUTIONS_PATH), "--data-type", "contributions", "--dry-run"])
    captured = capsys.readouterr()

    assert exit_code == 1
    assert "AL ingest failed" in captured.err
