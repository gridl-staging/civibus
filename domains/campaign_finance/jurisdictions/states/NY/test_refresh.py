...
from domains.campaign_finance.jurisdictions.states.NY import refresh as ny_refresh
from domains.campaign_finance.jurisdictions.states.NY.scraper.cli import run_ny_refresh

NOW = datetime(2026, 8, 25, 12, 0, tzinfo=timezone.utc)
CONFIG_PATH = Path(__file__).with_name("config.yaml")
EXPECTED_JOBS = (
    ("state-ny-contributions", "contributions"),
    ("state-ny-expenditures", "expenditures"),
    ("state-ny-independent_expenditures", "independent_expenditures"),
)
...
def test_build_refresh_jobs_matches_ny_package_api() -> None:
    config = _load_config()
    jobs = ny_refresh.build_refresh_jobs(config, RunnerParameters(), NOW)
    ...
    for job, (_key, data_type) in zip(jobs, EXPECTED_JOBS, strict=True):
        ...
        assert job.jurisdiction == "state/NY"
        assert job.cadence == source.update_frequency
        assert job.data_source_names == (source.name,)
        assert job.run_callable.func is run_ny_refresh
        assert job.run_callable.keywords == {"data_type": data_type, "download": True}

    alternate_jobs = ny_refresh.build_refresh_jobs(_load_config(jurisdiction_code="ZZ"), RunnerParameters(), NOW)
...
def test_build_refresh_jobs_omits_only_missing_source() -> None:
    config = _load_config()
    jobs = ny_refresh.build_refresh_jobs(_without_data_type(config, "expenditures"), RunnerParameters(), NOW)

    assert [job.key for job in jobs] == ["state-ny-contributions", "state-ny-independent_expenditures"]
