...
from domains.campaign_finance.jurisdictions.states.IN.refresh import build_refresh_jobs
...
def test_build_refresh_jobs_preserves_metadata_order_and_current_year() -> None:
    ...
    assert [job.run_callable.func for job in jobs] == [run_in_refresh, run_in_refresh]
    assert [job.run_callable.args for job in jobs] == [(), ()]
    assert [job.run_callable.keywords for job in jobs] == [
        {"data_type": "contributions", "download": True, "year": 2026},
        {"data_type": "expenditures", "download": True, "year": 2026},
    ]
    for job in jobs:
...
def test_build_refresh_jobs_emits_only_jobs_with_matching_sources() -> None:
    expected_key_by_source_name = {
        "IN IED Campaign Finance - Contributions": "state-in-contributions",
        "IN IED Campaign Finance - Expenditures": "state-in-expenditures",
    }
