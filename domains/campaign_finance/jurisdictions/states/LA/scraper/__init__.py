...
def _load_la_config() -> JurisdictionConfig:
    return load_state_config(_CONFIG_PATH, state_code="LA")


@lru_cache(maxsize=1)
def _load_la_data_source_blocks() -> tuple[DataSourceBlock, ...]:
    return build_data_source_blocks(_load_la_config())


def _load_data_source_for_data_type(data_type: str) -> DataSourceBlock:
    return load_data_source_for_data_type(
        _load_la_data_source_blocks(),
        data_type=data_type,
        state_code="LA",
    )
...
def load_supported_data_types() -> tuple[str, ...]:
    return load_block_supported_data_types(_load_la_data_source_blocks())
