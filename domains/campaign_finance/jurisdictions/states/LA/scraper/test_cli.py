...
def test_run_la_refresh_executes_path_mode(monkeypatch) -> None:
...
def test_run_la_refresh_requires_path_or_download() -> None:
    with pytest.raises(ValueError, match="requires either path or download mode"):
        cli.run_la_refresh(data_type="loans")
...
def test_main_dry_run_reports_row_count_without_db(monkeypatch, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    exit_code = cli.main(
        ["--year", "2026", "--path", str(_SAMPLE_CONTRIBUTIONS_PATH), "--data-type", "contributions", "--dry-run"]
    )
    captured = capsys.readouterr()

    assert exit_code == 0
    cli.get_connection.assert_not_called()
    assert "LA contributions dry-run: parsed 2 rows" in captured.out
