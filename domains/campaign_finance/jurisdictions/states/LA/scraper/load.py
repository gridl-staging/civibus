...
from .extract import extract_la_contribution, extract_la_expenditure, extract_la_loan
from .parse import parse_contributions, parse_expenditures, parse_loans

LOGGER = logging.getLogger(__name__)

_LA_DOMAIN = "campaign_finance"
_LA_JURISDICTION = "state/LA"
...
_LA_ENTITY_KEYS: dict[str, tuple[str, str]] = {
...
_LA_EXTRACT_FN = {
    "contributions": extract_la_contribution,
    "loans": extract_la_loan,
    "expenditures": extract_la_expenditure,
}
...
_LA_ENTITY_ROLES: dict[str, dict[str, str]] = {
    "contributions": {
        "person": "donor",
...
_LA_COUNTERPARTY_ROLES: dict[str, tuple[tuple[str, ...], tuple[str, ...]]] = {
    "contributions": (("donor",), ("donor_org",)),
    "loans": (("lender",), ("lender_org",)),
    "expenditures": (("payee",), ("payee_org",)),
}

_LA_TRANSACTION_KIND: dict[str, str] = {
...
def _la_source_record_key(row: Mapping[str, str | None]) -> str:
...
def _resolve_la_committee_id(conn: psycopg.Connection, committee_org: object) -> UUID:
    ...
    la_filer_number = _normalize_optional_text(committee_org.identifiers.get("la_filer_number"))
    if la_filer_number is not None:
        existing = find_organization_by_identifier(conn, "la_filer_number", la_filer_number)
        if existing is not None:
            return existing
    return resolve_organization_by_canonical_name(conn, committee_org)


def _load_la_transaction_entities(
    ...
    *,
    ...
    roles = _LA_ENTITY_ROLES[data_type]
    person_key, org_key = _LA_ENTITY_KEYS[data_type]
    ...
    committee_id = _resolve_la_committee_id(conn, committee)
    link_entity_source_and_optional_mailing_address(
        conn,
    ...
    organization = extracted.get(org_key)
    if organization is not None:
        org_id = resolve_organization_by_canonical_name(conn, organization)
    ...
    *,
    ...
    try:
        ...
        with conn.transaction():
            return _extract_and_load_la_row(conn, row, data_source_id, data_type=data_type)
    except Exception:
        LOGGER.exception("Failed loading LA %s row", data_type.rstrip("s"))
        return None
...
def _load_la_file(
    ...
    *,
    ...
    parser = _LA_PARSER_FN[data_type](Path(file_path), year=year, year_from=year_from)
    return _load_la_rows(
        conn,
        parser,
        data_source_id=data_source_id,
        data_type=data_type,
...
def _parse_optional_la_date(raw_value: str | None) -> date | None:
    ...
    try:
        ...
        pass
    for fmt in ("%m/%d/%Y %I:%M:%S %p", "%m/%d/%Y", "%m/%d/%y", "%Y-%m-%dT%H:%M:%S"):
        try:
            return datetime.strptime(normalized, fmt).date()
        except ValueError:
            continue
    raise ValueError(f"LA row has invalid date: {raw_value}")


def _required_la_text(value: str | None, field_name: str) -> str:
    normalized = _normalize_optional_text(value)
    if normalized is None:
        raise ValueError(f"LA row missing {field_name}")
    return normalized


def _parse_required_la_amount(raw_value: str | None, field_name: str) -> Decimal:
    normalized = _required_la_text(raw_value, field_name)
    try:
        return Decimal(normalized.replace(",", "").replace("$", ""))
    except InvalidOperation as exc:
        raise ValueError(f"LA row has invalid {field_name}: {raw_value}") from exc


def _build_la_filing_fec_id(row: Mapping[str, str | None], data_type: str) -> str:
    committee_id_col = _load_column_for_semantic_path(data_type, "committee.id")
    report_code_col = _load_column_for_semantic_path(data_type, "la.report_code")
    report_number_col = _load_column_for_semantic_path(data_type, "la.report_number")

    committee_id = _normalize_optional_text(row.get(committee_id_col)) or "no-filer"
    report_code = _normalize_optional_text(row.get(report_code_col)) or "no-report-code"
    report_number = _normalize_optional_text(row.get(report_number_col)) or "no-report-number"
    return f"LA-{committee_id}-{report_code}-{report_number}-{data_type}"


def _resolve_la_filing_committee_id(
    conn: psycopg.Connection,
    row: Mapping[str, str | None],
    data_type: str,
) -> UUID:
    extracted = _LA_EXTRACT_FN[data_type](dict(row))
    organization_id = _resolve_la_committee_id(conn, extracted["committee"])
    committee_id_col = _load_column_for_semantic_path(data_type, "committee.id")
    native_id = _normalize_optional_text(row.get(committee_id_col))
    if native_id is None:
        native_id = _build_la_filing_fec_id(row, data_type)
...
def _build_la_filing(
    ...
    *,
    ...
    filed_date_col = _load_column_for_semantic_path(data_type, "transaction.date")
    filed_date = _parse_optional_la_date(row.get(filed_date_col))

    return Filing(
        filing_fec_id=_build_la_filing_fec_id(row, data_type),
...
) -> _LAFilingLookupEntry:
    filing_fec_id = _build_la_filing_fec_id(row, data_type)
    existing = filing_lookup.get(filing_fec_id)

    if existing is None:
        committee_id = _resolve_la_filing_committee_id(conn, row, data_type)
        filing_source_record_id = source_record_id
    else:
        committee_id = existing.committee_id
        filing_source_record_id = existing.source_record_id
...
def _transaction_identifier(row: Mapping[str, str | None], *, data_type: str) -> str:
    del data_type
    return _la_source_record_key(row)


def _select_la_source_record_id(
    conn: psycopg.Connection,
    *,
    data_source_id: UUID,
...
def _resolve_la_transaction_address_id(
    conn: psycopg.Connection,
    *,
    source_record_id: UUID,
    data_type: str,
) -> UUID | None:
    address_role = _LA_ENTITY_ROLES[data_type]["address"]
    with conn.cursor() as cursor:
        cursor.execute(
            """
            SELECT entity_id
...
def _upsert_la_transaction_with_filing(
    conn: psycopg.Connection,
    row: Mapping[str, str | None],
    *,
    filing_id: UUID,
    ...
    person_roles, organization_roles = _LA_COUNTERPARTY_ROLES[data_type]
    person_id, organization_id = resolve_transaction_counterparty_ids(
        conn,
        source_record_id=source_record_id,
        person_roles=person_roles,
        organization_roles=organization_roles,
    )
    address_id = _resolve_la_transaction_address_id(
        conn,
        source_record_id=source_record_id,
        data_type=data_type,
    )
...
def _load_la_relational_transactions(
    conn: psycopg.Connection,
    rows: Iterable[Mapping[str, str | None]],
    *,
    data_source_id: UUID,
    ...
    for index, row in enumerate(rows, start=1):
        ...
        source_record_id = _select_la_source_record_id(
            conn,
            data_source_id=data_source_id,
            source_record_key=_la_source_record_key(row),
        )
        if source_record_id is None:
            continue
        ...
        try:
            if manages_outer:
                ensure_transaction_open(conn)
            with conn.transaction():
                filing_entry = _upsert_la_filing(
                    conn,
                    row,
                    source_record_id=source_record_id,
                    data_type=data_type,
                ...
                _upsert_la_transaction_with_filing(
                    conn,
                    row,
            ...
            relational_errors += 1
            LOGGER.exception("Failed linking LA %s row to filing", data_type.rstrip("s"))

    commit_managed_transaction(conn, manages_outer)
    return relational_errors


def _load_la_with_filings(
    conn: psycopg.Connection,
    fp: str | Path,
    *,
    data_type: str,
    ...
    validated_row_limit = validated_limit(limit)
    data_source_id = ensure_la_data_source(conn, data_type=data_type)

    load_result = _load_la_file(
        conn,
        fp,
        data_source_id=data_source_id,
        data_type=data_type,
    ...
    load_result.errors += _load_la_relational_transactions(
        conn,
        _LA_PARSER_FN[data_type](Path(fp), year=year, year_from=year_from),
        data_source_id=data_source_id,
        data_type=data_type,
        limit=validated_row_limit,
    )
...
def load_la_contributions_with_filings(
    conn: psycopg.Connection,
    fp: str | Path,
    *,
    year: int,
    ...
    return _load_la_with_filings(conn, fp, data_type="contributions", year=year, year_from=year_from, limit=limit)


def load_la_expenditures_with_filings(
    conn: psycopg.Connection,
    fp: str | Path,
    *,
    year: int,
    ...
    return _load_la_with_filings(conn, fp, data_type="expenditures", year=year, year_from=year_from, limit=limit)


def load_la_loans_with_filings(
    conn: psycopg.Connection,
    fp: str | Path,
    *,
    year: int,
    ...
    return _load_la_with_filings(conn, fp, data_type="loans", year=year, year_from=year_from, limit=limit)
