...
from domains.campaign_finance.jurisdictions.states.LA.scraper import download as la_download
from domains.campaign_finance.jurisdictions.states.LA.scraper.download import download_la_archive
...
def test_download_la_archive_uses_data_type_url_resolution(tmp_path: Path) -> None:
    ...
    with (
        patch(
            "domains.campaign_finance.jurisdictions.states.LA.scraper.download._load_bulk_download_url_for_data_type"
        ) as load_url,
...
def test_download_la_archive_streams_to_disk_and_returns_stable_path(tmp_path: Path) -> None:
    destination_dir = tmp_path / "downloads"
...
def test_download_la_archive_cleans_partial_files_on_failure(tmp_path: Path) -> None:
    destination_dir = tmp_path / "downloads"
    ...
    with patch("httpx.Client") as mock_client_type:
        ...
        with pytest.raises(httpx.ReadError, match="connection reset"):
            download_la_archive("loans", destination_dir)

    assert list(destination_dir.glob("*.part")) == []
    assert list(destination_dir.glob("LA_loans.zip")) == []


def test_download_la_archive_rejects_oversized_stream(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    destination_dir = tmp_path / "downloads"
    monkeypatch.setattr(la_download, "MAX_DOWNLOAD_BYTES", 4)
    ...
    with patch("httpx.Client") as mock_client_type:
        ...
        with pytest.raises(ValueError, match="exceeds the allowed size limit"):
            download_la_archive("contributions", destination_dir)
