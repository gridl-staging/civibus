...
class LACsvParser:
    """Streaming CSV parser for Louisiana bulk extracts."""
...
def _iter_member_readers(
    ...
    *,
    ...
    with zipfile.ZipFile(path) as archive:
        member_name = _select_la_member_name(archive, data_type=data_type, year=year)
        _validate_zip_member_size(archive, member_name)
        with archive.open(member_name) as member_stream:
            with io.TextIOWrapper(member_stream, encoding="utf-8-sig", errors="replace", newline="") as csv_file:
                reader = csv.DictReader(
                    csv_file,
                    restkey=_EXTRA_FIELD_SENTINEL,
                    restval=_MISSING_FIELD_SENTINEL,
...
def _member_matches_year(*, member_basename: str, data_type: str, year: int) -> bool:
    prefix = _LA_MEMBER_PREFIX_BY_TYPE.get(data_type)
    if prefix is None:
        raise ValueError(f"Unsupported LA data type: {data_type}")

    range_match = re.fullmatch(rf"{prefix}_(\d{{4}})_to_(\d{{4}})\.csv", member_basename)
    if range_match is not None:
        start_year = int(range_match.group(1))
        end_year = int(range_match.group(2))
        return start_year <= year <= end_year

    earlier_match = re.fullmatch(rf"{prefix}_(\d{{4}})_and_earlier\.csv", member_basename)
    if earlier_match is not None:
        return year <= int(earlier_match.group(1))

    return False


def _select_la_member_name(archive: zipfile.ZipFile, *, data_type: str, year: int) -> str:
    matches = [
        member.filename
        for member in archive.infolist()
        if not member.is_dir()
        and _member_matches_year(member_basename=Path(member.filename).name, data_type=data_type, year=year)
    ]

    if len(matches) != 1:
        raise ValueError(f"LA ZIP must contain exactly one {data_type} member for year {year} (found {len(matches)})")
...
def _validate_zip_member_size(archive: zipfile.ZipFile, member_name: str) -> None:
    ...
    if member_info.file_size > MAX_ZIP_MEMBER_BYTES:
        raise ValueError(
            f"LA ZIP member {Path(member_name).name!r} exceeds the allowed size limit of {MAX_ZIP_MEMBER_BYTES} bytes"
        )
...
def parse_contributions(path: Path, *, year: int, year_from: int | None = None) -> LACsvParser:
    return LACsvParser(
        path=path,
...
def parse_loans(path: Path, *, year: int, year_from: int | None = None) -> LACsvParser:
    return LACsvParser(
        path=path,
...
def parse_expenditures(path: Path, *, year: int, year_from: int | None = None) -> LACsvParser:
    return LACsvParser(
        path=path,
