"""
Stub summary for /Users/stuart/parallel_development/civibus_dev/mar22_pm_02_oh_state_pipeline/civibus_dev/domains/campaign_finance/jurisdictions/states/OH/scraper/parse.py.
"""
...
class OHCsvParser:
    """Iterate OH CSV rows as normalized dictionaries."""
        ...
        with self.path.open("r", encoding="utf-8", errors="replace", newline="") as csv_file:
            ...
            for raw_row in reader:
                if _is_malformed_row(raw_row):
                    self.skipped += 1
                    LOGGER.warning(
                        "Skipping malformed %s row in %s at line %d", self.row_label, self.path.name, reader.line_num
                    )
                    continue
...
def _normalize_row(raw_row: dict[object, object]) -> dict[str, str | None]:
    ...
    for key, value in raw_row.items():
        ...
        normalized_row[str(key)] = None if value in _NULL_SENTINELS else value

    return normalized_row


def parse_contributions(path: Path) -> OHCsvParser:
    return OHCsvParser(path=path, columns=CONTRIBUTION_COLUMNS, data_type="contributions", row_label="contribution")


def parse_expenditures(path: Path) -> OHCsvParser:
    return OHCsvParser(path=path, columns=EXPENDITURE_COLUMNS, data_type="expenditures", row_label="expenditure")
