"""
Stub summary for /Users/stuart/parallel_development/civibus_dev/MAR18_state_expansion_batch_2/civibus_dev/domains/campaign_finance/jurisdictions/states/MN/scraper/parse.py.
"""
...
def parse_contributions(path: Path) -> MNCsvParser:
    return MNCsvParser(path=path, columns=CONTRIBUTION_COLUMNS, row_label="contribution")


def parse_expenditures(path: Path) -> MNCsvParser:
    return MNCsvParser(path=path, columns=EXPENDITURE_COLUMNS, row_label="expenditure")


def parse_independent_expenditures(path: Path) -> MNCsvParser:
    return MNCsvParser(
        path=path,
