"""Texas campaign finance scraper config helpers."""
...
@lru_cache(maxsize=1)
def _load_tx_config() -> JurisdictionConfig:
    try:
        return load_jurisdiction_config(_CONFIG_PATH)
    except ValueError as error:
        raise RuntimeError(f"Could not load TX scraper config from {_CONFIG_PATH}: {error}") from error


@lru_cache(maxsize=1)
def _load_tx_data_source_blocks() -> tuple[_TXDataSourceBlock, ...]:
...
def _load_data_source_for_data_type(data_type: str) -> _TXDataSourceBlock:
    normalized_data_type = _normalize_data_type(data_type)

    for data_source in _load_tx_data_source_blocks():
...
def _load_column_for_semantic_path(data_type: str, semantic_path: str) -> str:
    ...
    if not matching_columns:
        raise RuntimeError(f"No TX field mapping found for data type {data_type!r} and semantic path {semantic_path!r}")
    if len(matching_columns) > 1:
        raise RuntimeError(
            f"Multiple TX field mappings found for data type {data_type!r} and semantic path {semantic_path!r}"
        )
...
def _load_bulk_download_url_for_data_type(data_type: str) -> str:
    ...
    if data_source.bulk_download_url is None:
        raise RuntimeError(f"TX config missing bulk_download_url for data type {data_type!r}")
