"""
Stub summary for /Users/stuart/parallel_development/civibus_dev/mar26_am_3_new_state_pipeline_builds/civibus_dev/domains/campaign_finance/jurisdictions/states/WI/scraper/parse.py.
"""
...
TRANSACTION_COLUMNS = _load_columns_for_data_type("transactions")
REPORT_COLUMNS = _load_columns_for_data_type("reports")
COMMITTEE_COLUMNS = _load_columns_for_data_type("committees")


class WICsvParser:
    """Iterate WI CSV rows as normalized dictionaries."""
...
def parse_transactions(path: Path) -> WICsvParser:
    return WICsvParser(path=path, columns=TRANSACTION_COLUMNS, row_label="transaction")


def parse_reports(path: Path) -> WICsvParser:
    return WICsvParser(path=path, columns=REPORT_COLUMNS, row_label="report")


def parse_committees(path: Path) -> WICsvParser:
    return WICsvParser(path=path, columns=COMMITTEE_COLUMNS, row_label="committee")
