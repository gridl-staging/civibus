"""
Stub summary for /Users/stuart/parallel_development/civibus_dev/mar26_am_3_new_state_pipeline_builds/civibus_dev/domains/campaign_finance/jurisdictions/states/WI/scraper/cli.py.
"""
...
from .download import download_wi_csv
from .load import LoadResult, load_wi_transactions_with_filings
from .parse import parse_committees, parse_reports, parse_transactions

_SUPPORTED_DATA_TYPES = ("transactions", "reports", "committees")
_LOADABLE_DATA_TYPES = ("transactions",)
...
def _build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Load Wisconsin campaign-finance Sunshine CSV data into Civibus")
    input_source_group = parser.add_mutually_exclusive_group(required=True)
    input_source_group.add_argument("--path", type=Path, help="Path to a local WI CSV export")
    input_source_group.add_argument(
        "--download", action="store_true", help="Download WI CSV export for the selected data type"
    )
    parser.add_argument(
        "--data-type", required=True, choices=list(_SUPPORTED_DATA_TYPES), help="WI data type to ingest"
    )
...
def _validate_loadable_data_type(data_type: str) -> str:
    normalized_data_type = _validate_data_type(data_type)
    if normalized_data_type not in _LOADABLE_DATA_TYPES:
        raise ValueError(f"WI {normalized_data_type} data type is supported for parse/dry-run only")
...
def _validate_refresh_args(args: argparse.Namespace) -> None:
    _validate_loadable_data_type(args.data_type)
    if args.path is None and not args.download:
        raise ValueError("WI refresh requires either path or download mode")
    if args.path is not None and args.download:
        raise ValueError("WI refresh accepts path or download mode, not both")


def _print_load_summary(result: LoadResult, data_type: str) -> None:
    print(
        f"WI {data_type} load complete: "
...
def _print_dry_run_summary(data_type: str, parsed_count: int) -> None:
    print(f"WI {data_type} dry-run: parsed {parsed_count} rows")
...
def _resolve_input_path(args: argparse.Namespace) -> tuple[Path, tempfile.TemporaryDirectory[str] | None]:
    ...
    temp_dir = tempfile.TemporaryDirectory(prefix=f"wi-{args.data_type}-")
    try:
        download_path = download_wi_csv(args.data_type, dest_dir=Path(temp_dir.name))
...
def _parser_for_data_type(data_type: str):
    normalized_data_type = _validate_data_type(data_type)
    if normalized_data_type == "transactions":
        return parse_transactions
    if normalized_data_type == "reports":
        return parse_reports
    return parse_committees
...
def _load_path(
    ...
    *,
    ...
    _validate_loadable_data_type(data_type)
    return load_wi_transactions_with_filings(connection, input_path, limit=limit)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.dry_run:
            input_path, temp_dir = _resolve_input_path(args)
            _print_dry_run_summary(args.data_type, _count_rows(input_path, data_type=args.data_type, limit=args.limit))
            return 0

        load_result = run_wi_refresh(
        ...
        print(f"WI ingest failed: {error}", file=sys.stderr)
        return 1
    finally:
        _cleanup_temp_dir(temp_dir)
