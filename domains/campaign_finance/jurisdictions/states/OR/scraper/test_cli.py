...
from domains.campaign_finance.jurisdictions.states.OR.scraper import cli
...
def test_build_argument_parser_accepts_all_or_data_types() -> None:
    parser = cli._build_argument_parser()
    for data_type in ("contributions", "expenditures"):
        args = parser.parse_args(["--path", "/tmp/sample.xls", "--data-type", data_type])
...
def test_build_argument_parser_rejects_path_and_download_together() -> None:
    with pytest.raises(SystemExit, match="2"):
        cli._build_argument_parser().parse_args(
            ["--path", "/tmp/sample.xls", "--download", "--data-type", "contributions"]
        )


def test_run_or_refresh_executes_path_mode(monkeypatch) -> None:
...
def test_run_or_refresh_requires_path_or_download() -> None:
    with pytest.raises(ValueError, match="requires either path or download mode"):
        cli.run_or_refresh(data_type="contributions")
...
def test_main_dry_run_reports_row_count_without_db(monkeypatch, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert "OR contributions dry-run: parsed 3 rows" in captured.out
