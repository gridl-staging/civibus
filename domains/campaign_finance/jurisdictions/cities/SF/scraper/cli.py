...
from .download import download_sf_csv
from .load import LoadResult, load_sf_transactions_with_filings
...
def _build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Load San Francisco campaign-finance CSV data into Civibus")
    input_source_group = parser.add_mutually_exclusive_group(required=True)
    input_source_group.add_argument("--path", type=Path, help="Path to a local SF CSV export")
    input_source_group.add_argument(
        "--download",
        action="store_true",
        help="Download current SF CSV export for the selected data type",
    )
    parser.add_argument(
        "--data-type",
        required=True,
        choices=list(_SUPPORTED_DATA_TYPES),
        help="SF data type to ingest",
    )
...
def _print_dry_run_summary(data_type: str, parsed_count: int) -> None:
    print(f"SF {data_type} dry-run: parsed {parsed_count} rows")
...
def _load_path(
    ...
    *,
    ...
    return load_sf_transactions_with_filings(connection, input_path, limit=limit)
...
def run_sf_refresh(
    *,
    ...
    if path is None and not download:
        raise ValueError("SF refresh requires either path or download mode")
    if path is not None and download:
        raise ValueError("SF refresh accepts path or download mode, not both")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"SF ingest failed: {error}", file=sys.stderr)
        return 1
