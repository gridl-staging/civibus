...
from .download import download_nyc_csv
from .load import LoadResult, load_nyc_transactions_with_filings
from .parse import parse_transactions

_SUPPORTED_DATA_TYPES = ("transactions",)
...
def _build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Load NYC campaign-finance CSV data into Civibus")
    input_source_group = parser.add_mutually_exclusive_group(required=True)
    input_source_group.add_argument("--path", type=Path, help="Path to a local NYC CSV export")
    input_source_group.add_argument(
        "--download",
        action="store_true",
        help="Download current NYC CSV export for the selected data type",
    )
    parser.add_argument(
        "--data-type",
        required=True,
        choices=list(_SUPPORTED_DATA_TYPES),
        help="NYC data type to ingest",
    )
...
def _validate_data_type(data_type: str) -> str:
    if data_type not in _SUPPORTED_DATA_TYPES:
        raise ValueError(f"Unsupported NYC data type: {data_type}")
    return data_type


def _print_load_summary(result: LoadResult, data_type: str) -> None:
    print(
        f"NYC {data_type} load complete: "
...
def _print_dry_run_summary(data_type: str, parsed_count: int) -> None:
    print(f"NYC {data_type} dry-run: parsed {parsed_count} rows")
...
def _resolve_input_path(args: argparse.Namespace) -> tuple[Path, tempfile.TemporaryDirectory[str] | None]:
    ...
    temp_dir = tempfile.TemporaryDirectory(prefix=f"nyc-{args.data_type}-")
    try:
        download_path = download_nyc_csv(args.data_type, dest_dir=Path(temp_dir.name))
...
def _count_rows(path: Path, *, limit: int | None) -> int:
    parser = parse_transactions(path)
    count = 0
...
def _load_path(
    ...
    *,
    ...
    return load_nyc_transactions_with_filings(connection, input_path, limit=limit)
...
def run_nyc_refresh(
    *,
    ...
    """Public API for the refresh runner."""
    _validate_data_type(data_type)
    if path is None and not download:
        raise ValueError("NYC refresh requires either path or download mode")
    if path is not None and download:
        raise ValueError("NYC refresh accepts path or download mode, not both")
    ...
    try:
        ...
        load_result = _load_path(connection, input_path, limit=limit)
        connection.commit()
        return load_result
    finally:
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.dry_run:
            ...
            try:
                _print_dry_run_summary(
                    args.data_type,
                    _count_rows(input_path, limit=args.limit),
                )
            finally:
                if temp_dir is not None:
                    temp_dir.cleanup()
        ...
        print(f"NYC ingest failed: {error}", file=sys.stderr)
        return 1
