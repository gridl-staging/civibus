...
@lru_cache(maxsize=1)
def _load_nyc_config() -> JurisdictionConfig:
    try:
        return load_jurisdiction_config(_CONFIG_PATH)
    except ValueError as error:
        raise RuntimeError(f"Could not load NYC scraper config from {_CONFIG_PATH}: {error}") from error


@lru_cache(maxsize=1)
def _load_nyc_data_source_blocks() -> tuple[_NYCDataSourceBlock, ...]:
...
def _load_data_source_for_data_type(data_type: str) -> _NYCDataSourceBlock:
    normalized_data_type = _normalize_data_type(data_type)

    for data_source in _load_nyc_data_source_blocks():
...
def _load_bulk_download_url_for_data_type(data_type: str) -> str:
    ...
    raise RuntimeError(f"NYC config missing bulk_download_url for data type {data_type!r}")
