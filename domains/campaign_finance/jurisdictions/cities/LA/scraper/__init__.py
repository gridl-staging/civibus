...
def _load_la_config() -> JurisdictionConfig:
    try:
        ...
        raise RuntimeError(f"Could not load LA scraper config from {_CONFIG_PATH}: {error}") from error


@lru_cache(maxsize=1)
def _load_la_data_source_blocks() -> tuple[_LADataSourceBlock, ...]:
...
def _load_data_source_for_data_type(data_type: str) -> _LADataSourceBlock:
    normalized_data_type = _normalize_data_type(data_type)

    for data_source in _load_la_data_source_blocks():
...
def _load_bulk_download_url_for_data_type(data_type: str) -> str:
    ...
    raise RuntimeError(f"LA config missing bulk_download_url for data type {data_type!r}")
