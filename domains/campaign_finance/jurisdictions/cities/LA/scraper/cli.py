...
from .download import download_la_csv
from .load import LoadResult, load_la_transactions_with_filings
...
def _build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Load Los Angeles campaign-finance CSV data into Civibus")
    input_source_group = parser.add_mutually_exclusive_group(required=True)
    input_source_group.add_argument("--path", type=Path, help="Path to a local LA CSV export")
    input_source_group.add_argument(
        "--download",
        action="store_true",
        help="Download current LA CSV export for the selected data type",
    )
    parser.add_argument(
        "--data-type",
        required=True,
...
def _resolve_input_path(args: argparse.Namespace) -> tuple[Path, tempfile.TemporaryDirectory[str] | None]:
    ...
    try:
        download_path = download_la_csv(args.data_type, dest_dir=Path(temp_dir.name))
...
def _load_path(
    ...
    *,
    ...
    return load_la_transactions_with_filings(connection, input_path, limit=limit)
