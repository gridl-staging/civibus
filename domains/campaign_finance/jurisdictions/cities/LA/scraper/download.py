...
def build_la_download_url(data_type: str, *, limit: int = SODA_PAGE_SIZE, offset: int = 0) -> str:
...
def _stream_la_pages_to_path(data_type: str, destination_path: Path, *, limit: int | None = None) -> None:
        ...
        with temporary_download_path.open("w", encoding="utf-8", newline="") as destination_file:
            ...
            with httpx.Client(timeout=REQUEST_TIMEOUT_SECONDS) as http_client:
                while True:
                    ...
                    download_url = build_la_download_url(data_type, limit=page_size, offset=offset)
                    ...
                    if expected_header is None:
                        ...
                        raise ValueError("LA download page header changed across paginated response")
...
def download_la_csv(data_type: str, dest_dir: Path, *, limit: int | None = None) -> Path:
    destination_path = dest_dir / f"la_{data_type.strip().lower()}.csv"
    _stream_la_pages_to_path(data_type, destination_path, limit=limit)
