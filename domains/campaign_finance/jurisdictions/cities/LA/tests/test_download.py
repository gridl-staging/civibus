...
from domains.campaign_finance.jurisdictions.cities.LA.scraper import (
    _load_bulk_download_url_for_data_type,
    _load_columns_for_data_type,
    _load_data_source_for_data_type,
    _load_la_config,
)
from domains.campaign_finance.jurisdictions.cities.LA.scraper import download as la_download
from domains.campaign_finance.jurisdictions.cities.LA.scraper.download import (
    SODA_ORDER_BY,
    SODA_PAGE_SIZE,
    build_la_download_url,
    download_la_csv,
)
...
def test_load_la_config_and_contributions_source_from_yaml() -> None:
    config = _load_la_config()
    data_source = _load_data_source_for_data_type("  TRANSACTIONS  ")

    assert config.jurisdiction.code == "LA"
    assert data_source.name == "LA Ethics Campaign Contributions"
...
def test_load_columns_for_transactions_derive_from_config_order() -> None:
    ...
    assert columns[0] == "con_date"
    assert columns[-1] == "memo"


def test_load_bulk_download_url_for_transactions_uses_config() -> None:
    assert _load_bulk_download_url_for_data_type("transactions") == "https://data.lacity.org/resource/m6g2-gc6c.csv"


def test_build_la_download_url_uses_contributions_dataset_with_pagination() -> None:
    url = build_la_download_url("transactions", limit=500, offset=1000)

    assert "m6g2-gc6c.csv" in url
...
def test_build_la_download_url_rejects_unsupported_data_type() -> None:
    with pytest.raises(ValueError, match="Unsupported LA data type"):
        build_la_download_url("pledges")


def test_download_la_csv_merges_pages_header_once_and_names_destination(tmp_path: Path) -> None:
    ...
    with patch("httpx.Client") as mock_client_type:
        ...
        with patch.object(la_download, "SODA_PAGE_SIZE", 2):
            output_path = download_la_csv("transactions", destination_dir)

    assert output_path == destination_dir / "la_transactions.csv"
    assert output_path.read_text(encoding="utf-8") == "id,amount\n1,100.00\n2,250.00\n3,500.00\n"


def test_download_la_csv_removes_partial_file_when_stream_raises(tmp_path: Path) -> None:
    destination_dir = tmp_path / "downloads"
    ...
    with patch("httpx.Client") as mock_client_type:
        ...
        with pytest.raises(httpx.ReadError, match="stream interrupted"):
            download_la_csv("transactions", destination_dir)

    assert list(destination_dir.iterdir()) == []


def test_download_la_csv_paginates_by_csv_rows_with_quoted_newlines(tmp_path: Path) -> None:
    destination_dir = tmp_path / "downloads"
...
def test_build_la_download_url_uses_stable_soda_order() -> None:
    url = build_la_download_url("transactions")
    assert "$order=:id" in url
