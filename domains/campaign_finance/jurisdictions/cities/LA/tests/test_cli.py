"""Unit tests for the LA CLI entrypoint."""
...
from domains.campaign_finance.jurisdictions.cities.LA.scraper import cli
...
def test_main_loads_path_and_prints_summary(
    ...
    monkeypatch.setattr(cli, "load_la_transactions_with_filings", load_with_filings)
    ...
    assert "LA transactions load complete" in captured.out
...
def test_main_download_mode_resolves_path_and_loads(
    ...
    connection = MagicMock()
    load_result = _build_load_result()
    downloaded_path = tmp_path / "la_transactions.csv"

    monkeypatch.setattr(cli, "get_connection", MagicMock(return_value=connection))
    download_la_csv = MagicMock(return_value=downloaded_path)
    monkeypatch.setattr(cli, "download_la_csv", download_la_csv)
    load_with_filings = MagicMock(return_value=load_result)
    monkeypatch.setattr(cli, "load_la_transactions_with_filings", load_with_filings)
    ...
    assert exit_code == 0
    download_la_csv.assert_called_once_with("transactions", dest_dir=ANY)
...
def test_main_dry_run_uses_path_without_db(
    ...
    monkeypatch.setattr(cli, "load_la_transactions_with_filings", MagicMock())
    ...
    cli.load_la_transactions_with_filings.assert_not_called()
    assert "LA transactions dry-run: parsed 3 rows" in captured.out
...
def test_main_returns_error_and_closes_connection_when_load_fails(
    ...
    assert exit_code == 1
    assert "LA ingest failed: load failed" in captured.err
    connection.commit.assert_not_called()
    connection.close.assert_called_once_with()


def test_run_la_refresh_executes_path_mode(monkeypatch: pytest.MonkeyPatch) -> None:
    connection = MagicMock()
    load_result = _build_load_result()
...
def test_run_la_refresh_download_mode(monkeypatch: pytest.MonkeyPatch) -> None:
    connection = MagicMock()
    load_result = _build_load_result()

    monkeypatch.setattr(cli, "get_connection", MagicMock(return_value=connection))
    download_la_csv = MagicMock(return_value=Path("/tmp/la_transactions.csv"))
    monkeypatch.setattr(cli, "download_la_csv", download_la_csv)
    load_with_filings = MagicMock(return_value=load_result)
    monkeypatch.setattr(cli, "load_la_transactions_with_filings", load_with_filings)

    result = cli.run_la_refresh(data_type="transactions", download=True)

    assert result == load_result
    download_la_csv.assert_called_once()
    connection.commit.assert_called_once_with()
    connection.close.assert_called_once_with()


def test_run_la_refresh_rejects_unsupported_data_type(monkeypatch: pytest.MonkeyPatch) -> None:
    get_connection = MagicMock()
    monkeypatch.setattr(cli, "get_connection", get_connection)

    with pytest.raises(ValueError, match="Unsupported LA data type: receipts"):
        cli.run_la_refresh(data_type="receipts", path=_SAMPLE_TRANSACTIONS_PATH)
