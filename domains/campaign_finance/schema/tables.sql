...
--
-- Schema ownership
-- This file owns the campaign-finance tables, including these launch-critical tables:
--   - cf.committee
...
CREATE TABLE cf.committee_summary (
    ...
    derived_top_donors                      JSONB,
    derived_top_vendors                     JSONB,
    derived_spend_categories                JSONB,
    derived_filing_breakdown                JSONB,
    source_record_id                        UUID REFERENCES core.source_record(id),
    created_at                              TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at                              TIMESTAMPTZ NOT NULL DEFAULT NOW(),
...
CREATE INDEX idx_donor_search_rollup_search_text_trgm
...
CREATE TABLE cf.candidate (
    ...
    total_disbursements      NUMERIC(14,2),
    cash_on_hand             NUMERIC(14,2),
    candidate_contrib        NUMERIC(14,2),
    candidate_loans          NUMERIC(14,2),
    candidate_loan_repay     NUMERIC(14,2),
...
CREATE TABLE cf.contribution_limit_rules (
    id                       UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
CREATE UNIQUE INDEX uq_contribution_limit_rules_identity
...
-- Temporal integrity: no two rules for one jurisdiction/dimension tuple may hold
-- overlapping effectivity periods. Added AFTER the identity index so an exact-tuple
-- duplicate reports uq_contribution_limit_rules_identity (the ON CONFLICT arbiter a
-- loader would upsert against, which an EXCLUDE constraint cannot serve), while a
-- partial overlap or an unknown row (unbounded daterange) coexisting with a known
-- rule reports this constraint. sunset_date is exclusive: it is the first date the
-- rule no longer applies, so a successor may take effect on that same date. Unknown
-- rows carry NULL dates, so daterange(NULL, NULL, '[)') is unbounded and overlaps any
-- sibling for the same tuple.
