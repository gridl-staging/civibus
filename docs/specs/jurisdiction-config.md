...
Within the campaign-finance region lifecycle, `config.yaml` is primarily the **Source Contract Translation** artifact. It captures machine-readable source facts and structured legal facts, but it is not the complete lifecycle record for a region. See [`docs/specs/campaign-finance-region-lifecycle.md`](./campaign-finance-region-lifecycle.md).
