...
def _create_replacement_table(cursor: psycopg.Cursor, replacement_name: str) -> None:
    ...
    cursor.execute(
        sql.SQL(
            "INSERT INTO cf.{} ("
            "donor_key, contributor_name, contributor_employer, contributor_occupation, contributor_city, "
            "contributor_state, normalized_zip5, jurisdiction, search_text, total_amount, transaction_count, "
...
def _index_replacement_table(cursor: psycopg.Cursor, replacement_name: str) -> None:
    replacement_identifier = sql.Identifier(replacement_name)
...
def _swap_replacement_table(cursor: psycopg.Cursor, replacement_name: str) -> None:
...
def rebuild_donor_search_rollup(connection: psycopg.Connection) -> DonorRollupBuildResult:
    ...
    with connection.transaction():
        with connection.cursor() as cursor:
            ...
            _create_replacement_table(cursor, replacement_name)
            row_count = cursor.rowcount
            _index_replacement_table(cursor, replacement_name)
            ...
            _swap_replacement_table(cursor, replacement_name)
